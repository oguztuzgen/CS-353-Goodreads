...
if (isset($_POST['submit'])) {
	...
	if (!array_filter($errors) && empty($image_error)) {
		...
		$sql = "UPDATE user SET first_name=\"$profile_first_name\", last_name=\"$profile_last_name\", birth_date=\"$profile_birth_date\", about=\"$profile_about\", address=\"$profile_address\", profile_picture=\"$upload_path\"
			where user_id = " . $uid . ";";
...
<div class=" brand-dark" style="padding: 150px 7%;">
	<?php echo "<form action=\"edit_profile.php?user_id=$uid\" method=\"POST\" enctype=\"multipart/form-data\">"; ?>
		<div class="row">
			<div class="col s3">
			...
			<div class="col s9" style="background: #AAD5FF">
				...
				<div class="col s12">
					<?php
						...
						echo "<textarea name=\"profile_about\" value=\"$profile_about\" style=\"float: left; margin-top: 15px; padding: 10px; width: 100%; height: 100%; background-color:#7fa1bf; border-color: white;\" class=\"text\" placeholder=\"About\"></textarea>";
						// $error = $errors['profile_about']; echo "<p class=\"red-text\">$error</p>";

						echo "<textarea name=\"profile_address\" value=\"$profile_address\" style=\"float: left; margin-top: 15px; padding: 10px; width: 100%; height: 100%; background-color:#7fa1bf; border-color: white;\" class=\"text\" placeholder=\"Address\"></textarea>";
