...
if(isset($_POST['send'])){
    ...
    $sql = "insert into error_report(report_message, report_type) values('". $_POST['description'] ."', 'USER');";
    // echo $sql;
    ...
    $sql2 = "insert into reports(user_id, book_id, report_id) values (".$_SESSION['user_id']." , 1 , " . $r_id['max(report_id)'] . "); ";
    // echo $sql;
    if(mysqli_query($conn, $sql2)){
    ?>
    <p class="text " style="color: green; text-align:center;"> Report Succesfully Sent Turn Back to Index Page or Fill in Another Report About the User </p>
    
<?php
}

else{
    echo '<p class="text " style="color: red; text-align:center;"> ' .$sql2. ' </p>';
}
}
?>
